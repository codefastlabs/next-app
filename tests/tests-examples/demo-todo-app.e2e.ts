...
async function checkNumberOfTodosInLocalStorage(page: Page, expected: number): Promise<JSHandle<boolean>> {
  return await page.waitForFunction((e) => {
    const todos = JSON.parse(localStorage.getItem('react-todos') ?? '[]') as object[];
...
async function checkNumberOfCompletedTodosInLocalStorage(page: Page, expected: number): Promise<JSHandle<boolean>> {
  return await page.waitForFunction((e) => {
    const todos = JSON.parse(localStorage.getItem('react-todos') ?? '[]') as { completed: boolean }[];

    return todos.filter((todo) => todo.completed).length === e;
  }, expected);
}

async function checkTodosInLocalStorage(page: Page, title: string): Promise<JSHandle<boolean>> {
  return await page.waitForFunction((t) => {
    const todos = JSON.parse(localStorage.getItem('react-todos') ?? '[]') as { title: string }[];
